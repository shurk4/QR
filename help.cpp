...
void help::showTextfilesHelp()
{
    ...
    ui->textBrowser->append("<h4>Объединение товаров:</h4>");
    ui->textBrowser->append("Для объединения товаров надо выделить необходимые товары/итемы и нажать кнопку объединить."
                            "После этого в открывшемся окне ввести имя, под которым объединённые товары будут отображаться в программе, и нажать ок.\n"
                            "Выделить итемы можно протяжкой, через shift выбрав первый и последний элемент диапазона или через ctrl выбирая каждый нужный элемент по отдельности.");
    ui->textBrowser->append("В окне ввода имени имя можно скопировать из списка выбранных позиций для объединения.");
